    ...
    for ps_ip, gpu in zip(ps_ips, total_ps_gpus):
        ...
        for cuda_id in range(len(gpu)):
            for _  in range(gpu[cuda_id]):
                ...
                ps_cmd = f" python {yaml_conf['exp_path']}/{yaml_conf['aggregator_entry']} {conf_script} --this_rank={ps_rank_id} --num_executors={worker_processes_per_ps_processes} --num_aggregators={total_ps_gpu_processes} --executor_configs={executor_configs[ps_rank_id - 1]} --aggregator_configs={aggregator_configs}"
                ps_rank_id += 1
